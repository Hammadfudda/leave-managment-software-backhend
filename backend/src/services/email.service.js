...
export const templates = {
  ...
  approved: (request) =>
    layout(
      ...
      `<p>
         Your <strong>${request.leaveType}</strong> leave from
         ${new Date(request.startDate).toDateString()} to
         ${new Date(request.endDate).toDateString()} has been fully approved.
       </p>`,
    ),
  ...
  rejected: (request, comment) =>
    layout(
      ...
      `<p>
         Your <strong>${request.leaveType}</strong> leave from
         ${new Date(request.startDate).toDateString()} to
         ${new Date(request.endDate).toDateString()} was rejected.
       </p>
  ...
  cancelled: (request) =>
    layout(
      ...
      `<p>
         The <strong>${request.leaveType}</strong> leave request for
         ${request.employeeName} has been cancelled.
       </p>`,
    ),

...
import { authenticateSuperAdmin, loadSuperAdmin } from '../middleware/superAdminAuth.js';
import { loginLimiter } from '../middleware/rateLimit.js';

const router = Router();
router.post('/setup', loginLimiter, superAdmin.setupFirstSuperAdmin);
router.post('/login', loginLimiter, superAdmin.login);
router.use(authenticateSuperAdmin, loadSuperAdmin);
router.get('/me', superAdmin.me);
router.get('/organizations', superAdmin.listOrganizations);
router.post('/organizations', clientAccount.createOrganizationWithTemporaryPassword);
router.patch('/organizations/:id/status', superAdmin.updateOrganizationStatus);
